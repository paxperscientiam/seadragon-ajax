...
(function () {
    ...
    var hasCssTransitions = typeof docElmtStyle[cssTransitionProperty] !== "undefined";
    ...
    var IE_MATRIX_FILTER_REGEXP = new RegExp(IE_MATRIX_FILTER + "\\(.*?\\)", 'g');
    ...
    SeadragonDrawer = Seadragon.Drawer = function (source, viewport, elmt) {
        ...
        function loadTile(tile, time) {
            tile.loading = imageLoader.loadImage(tile.url, SeadragonUtils.createCallback(null, onTileLoad, tile, time));
        }
        ...
        function updateActual() {
            ...
            for (var level = highestLevel; level >= lowestLevel; level--) {
                var drawLevel = false;
                var renderPixelRatioC = _deltaPixelsFromPoints(_getPixelRatio(level), true).x; // note the .x!
